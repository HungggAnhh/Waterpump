...
export default function PageTasksScreen() {
  ...
  useEffect(() => {
    ...
    const handleTaskUpdated = (updatedTask: Task) => {
      setTasks(prev => {
        ...
        const pId = updatedTask.page_id || (existingTask ? existingTask.page_id : null);

        if (pId === parseInt(pageId as string)) {
          const exists = prev.some(t => t.id === updatedTask.id);
          if (!exists) {
            if (!updatedTask.page_id) return prev;
          ...
          return prev.map(t => {
            if (t.id === updatedTask.id) {
              if (user?.role !== 'admin' && updatedTask.assigned_to !== undefined && updatedTask.assigned_to !== user?.id) {
                return { ...t, _remove: true };
              }
              if (updatedTask.assignees) {
                return { ...t, ...updatedTask, assignees: updatedTask.assignees };
              }
              if (!updatedTask.page_id && 'user_id' in updatedTask) {
            ...
            return t;
          }).filter(t => !(t as any)._remove);
        } else {
          if (updatedTask.page_id) {
  ...
  const handleOpenCreateModal = () => {
    ...
    setFormSelectedUsers(workspaceMembers.map(m => m.id)); // Default to select all
    setFormDeadline('');
  ...
  const handleUpdateTask = async () => {
    ...
    try {
      ...
      if (result.status === 'success') {
        ...
        setFormError(result.message || 'Lỗi cập nhật.');
      }
    } catch (err) {
      console.error(err);
